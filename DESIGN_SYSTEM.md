...
#### 播客/访谈 (Podcast & Video) - "混合媒体画廊" (Hybrid Media Gallery)
我们采用 **"Media Agnostic"** (媒体中立) 的设计策略，在同一列表中兼容音频与视频。
*   **视觉隐喻**:
    *   **音频 (Audio)**: **黑胶唱片 (Vinyl)** 隐喻。正方形封面，表面带有微妙的光泽 (`gradient`)，悬停时显示 `Play` 图标。
    *   **视频 (Video)**: **胶片 (Film Strip)** 隐喻。正方形封面，上下边缘带有 **胶片齿孔 (Sprockets)** 装饰，悬停时显示 `Film` 图标。
*   **目的**: 保持列表的整齐统一 (Aspect Ratio 1:1)，同时通过微小的视觉线索区分内容载体。
